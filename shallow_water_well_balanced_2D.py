...
def solve_well_balanced(Lx=1.0, Ly=1.0, nx=80, ny=80, T=0.01, cfl=0.15, save_path="sw_wb_2d.png"):
    ...
    b = 0.15 * np.exp(-50 * ((X - 0.5)**2 + (Y - 0.5)**2))
    H = 1.0
    h0 = np.maximum(H - b, 1e-6)
    ...
    while t < T:
        ...
        U_new[1, 1:-1, 1:-1] += dt * Sx[1:-1, 1:-1]
        U_new[2, 1:-1, 1:-1] += dt * Sy[1:-1, 1:-1]
    ...
    cs = plt.contourf(X, Y, surface, levels=30)
    plt.colorbar(cs, label="Free surface")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("2D Well-Balanced Shallow Water")
    plt.tight_layout()
